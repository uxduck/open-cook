...
import { ArrowLeft, ChefHat, Copy, Link2, LogIn } from "lucide-react";
...
import {
  buildCookbookRecipeHead,
  buildRecipeJsonLd,
  siteOrigin,
} from "../lib/recipeOg";
import { getCookbookRecipe } from "../server/cookbook";
import { Button, inlineStatusClassName, pageContainerClassName } from "../ui";

export const Route = createFileRoute("/c/$slug/r/$recipeId")({
  loader: async ({ params }) => {
    const result = await getCookbookRecipe({
      data: { recipeId: params.recipeId, slug: params.slug },
    });
    if (!result) {
      throw notFound();
    }
  ...
  head: ({ loaderData, params }) =>
    buildCookbookRecipeHead(loaderData ?? null, {
      recipeId: params.recipeId,
      slug: params.slug,
    }),
  component: CookbookRecipeRoute,
  notFoundComponent: CookbookRecipeNotFound,
});

function CookbookRecipeRoute() {
  ...
  async function saveCopy() {
    ...
    try {
      const copy = await api.copyCookbookRecipe(result.cookbook.slug, result.recipe.id);
    ...
    <main className="min-h-screen text-(--color-pop-ink) [background:radial-gradient(circle_at_18%_14%,rgba(120,164,110,0.1),transparent_28%),#f7f8f2]">
      ...
      {result ? (
        <section className={`${detailPanelClassName} ${pageContainerClassName}`}>
          {result.cookbook.visibility === "public" ? (
            <JsonLd
              data={buildRecipeJsonLd(
                result.recipe,
                `${siteOrigin}/c/${encodeURIComponent(
                  result.cookbook.slug,
                )}/r/${encodeURIComponent(result.recipe.id)}`,
              )}
            />
          ) : null}
          <div className={detailToolbarClassName}>
            <div className={detailStatusClassName}>
              <Link2 size={16} />
              <span>
                {result.recipe.title} from {result.cookbook.title}
              </span>
            </div>
            <div className={detailActionsClassName}>
              <Button
                onClick={() =>
                  navigate({
                    to: "/c/$slug",
                    params: { slug: result.cookbook.slug },
                  })
                }
                size="sm"
                variant="secondary"
          ...
          <ReadOnlyRecipeContent recipe={result.recipe} />
          {status ? <p className={inlineStatusClassName}>{status}</p> : null}
        </section>
      ) : (
        <RecipeMissing onBack={() => navigate({ to: session ? "/app" : "/" })} />
      )}
    </main>
  );
}

function CookbookRecipeNotFound() {
  const navigate = useNavigate();
  const { session } = useSession();
  return <RecipeMissing onBack={() => navigate({ to: session ? "/app" : "/" })} />;
}

function RecipeMissing({ onBack }: { onBack: () => void }) {
  return (
    <section className="flex min-h-[60vh] items-center justify-center px-5 py-16">
      <div className="flex w-full max-w-md flex-col items-center gap-3 rounded-2xl border-2 border-(--color-ink) bg-(--color-panel) px-6 py-14 text-center shadow-[5px_5px_0_0_var(--color-ink)]">
        ...
        <p className="max-w-sm text-[14px] leading-snug text-(--color-fog)">
          This recipe is not available through that cookbook link.
        </p>
